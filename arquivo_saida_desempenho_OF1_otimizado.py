...
# Objetivo do código: cálculo do desempenho do método OF1 otimizado para a estimação da amplitude, fase ou pedestal pela validação cruzada K-Fold.
...
1) Instrução para salvar em arquivos os dados estatísticos do desempenho do método OF1 otimizado.
...
2) Função para o cálculo do desempenho do método OF1 otimizado pelo Erro Médio de Estimação (EME).
...
3) Função para o cálculo do desempenho do método OF1 otimizado pelo Erro Médio Quadrático (Mean Squared Error - MSE).
Entrada: número de elementos presentes em cada bloco e lista do erro de estimação para cada bloco do K-Fold.
Saída: média do MSE, variância do MSE e desvio padrão do MSE.

4) Função para o cálculo do desempenho do método OF1 otimizado pelo Erro Médio Absoluto (Mean Absolute Error - MAE).
Entrada: número de elementos presentes em cada bloco e lista do erro de estimação para cada bloco do K-Fold.
Saída: média do MAE, variância do MAE e desvio padrão do MAE.

5) Função para o cálculo do desempenho do método OF1 otimizado pela Relação Sinal-Ruído (Signal-to-Noise Ratio - SNR).
...
6) Função para o cálculo do desempenho do método OF1 otimizado pelo desvio padrão (DP).
Entrada: número de elementos presentes em cada bloco e lista do erro de estimação para cada bloco do K-Fold.
Saída: valor do desvio padrão de cada bloco.

7) Instrução da validação cruzada K-Fold adaptada para o cálculo do desempenho do método OF1 otimizado.
...
titulo_programa = colored("Geração de arquivos de saída pela técnica de validação cruzada K-Fold para a estimação da amplitude, fase ou pedestal pelo método Optimal Filtering (OF1 Otimizado):\n", attrs=["bold"])

# Impressão do título do programa.
print(titulo_programa)

### --------------- 1) INSTRUÇÃO PARA SALVAR OS DADOS ESTATÍSTICOS DO K-FOLD PELO MÉTODO OF1 OTIMIZADO ------------------------------ ###

# Definição da instrução para salvar os dados estatísticos do desempenho do método OF1 otimizado em arquivo de saída.
def arquivo_saida_dados_desempenho_OF1_otimizado(parametro, n_ocupacao, n_janelamento_ideal, media_dado_desempenho, var_dado_desempenho, DP_dado_desempenho, mecanismo_desempenho):
    ...
    pasta_saida = f"K_Fold_{parametro}_{mecanismo_desempenho}_Desempenho_OF1_Otimizado_OC"
    ...
    arquivo_saida = f"k_fold_{parametro}_{mecanismo_desempenho}_desempenho_OF1_otimizado_J_{n_janelamento_ideal}.txt"
...
### ----------- 7) INSTRUÇÃO PARA A VALIDAÇÃO CRUZADA K-FOLD ADAPTADA PARA O CÁLCULO DO DESEMPENHO DO MÉTODO OF1 OTIMIZADO ---------- ###

# Definição da instrução da técnica de validação cruzada K-Fold para o cálculo do desempenho do método OF1 otimizado.
def K_fold_desempenho_OF1_otimizado(parametro, n_ocupacao, n_janelamento_ideal, opcao_avaliacao_desempenho, Matriz_Pulsos_Sinais_Janelado, vetor_parametro_referencia_janelado, valor_minimo_amplitude_processo_fase,  vetor_amplitude_referencia_janelado):
    ...
    for indice_teste in range(0, len(blocos_pulsos_sinais)):
        ...
        # A variável bloco_lista_erro_parametro recebe o valor de retorno da função metodo_OF1_simples.
        bloco_lista_erro_parametro = metodo_OF1_otimizado(parametro, n_janelamento_ideal, bloco_treino_pulsos_sinais, bloco_teste_pulsos_sinais, bloco_teste_parametro_referencia, valor_minimo_amplitude_processo_fase, bloco_treino_amplitude_referencia)
    ...
    # Salva as informações dos dados estatísticos da análise do desempenho do método OF1 otimizado.
    arquivo_saida_dados_desempenho_OF1_otimizado(parametro, n_ocupacao, n_janelamento_ideal, media_desempenho, var_desempenho, DP_desempenho, mecanismo_desempenho)   
...
def principal_desempenho_OF1_otimizado():
    ...
    for n_ocupacao in tqdm(range(ocupacao_inicial, ocupacao_final+1, incremento_ocupacao)):
        ...
        K_fold_desempenho_OF1_otimizado(parametro, n_ocupacao, n_janelamento_ideal, opcao_avaliacao_desempenho, Matriz_Pulsos_Sinais_Janelado, vetor_parametro_referencia_janelado, valor_minimo_amplitude_processo_fase, vetor_amplitude_referencia_janelado)
...
principal_desempenho_OF1_otimizado()  
