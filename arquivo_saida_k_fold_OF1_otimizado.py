...
# Objetivo do código: implementação da validação cruzada K-Fold para o método OF1 otimizado para a estimação da amplitude, fase ou pedestal.
...
titulo_programa = colored("Geração de arquivos de saída pela técnica de validação cruzada K-Fold para o método Optimal Filtering (OF1 Otimizado):\n", attrs=["bold"])
...
# Definição da instrução para salvar as médias dos dados estatísticos da validação cruzada K-Fold em arquivo de saída para o método OF1 otimizado.
def arquivo_saida_dados_estatisticos_k_fold_erro_OF1_otimizado(parametro, n_ocupacao, n_janelamento, media_dado_erro, var_dado_erro, DP_dado_erro, dado):
    ...
    pasta_saida = f"K_Fold_{parametro}_{dado}_Dados_Estatisticos_OF1_Otimizado_OC"
    ...
    arquivo_saida = f"k_fold_{parametro}_{dado}_dados_estatisticos_OF1_otimizado_OC_{n_ocupacao}.txt"
...
### ------------------------- 2) INSTRUÇÃO PARA A VALIDAÇÃO CRUZADA K-FOLD PELO MÉTODO OF1 OTIMIZADO -------------------------------- ###

# Definição da instrução da técnica de validação cruzada K-Fold para o método OF1 otimizado.
def K_fold_OF2_otimizado(parametro, n_ocupacao, n_janelamento, Matriz_Pulsos_Sinais_Janelado, vetor_parametro_referencia_janelado, valor_minimo_amplitude_estimada_processo_fase, vetor_amplitude_referencia_janelado):
    ...
    for indice_teste in range(0, len(blocos_pulsos_sinais)):
        ...
        # A variável bloco_lista_erro_estimacao_parametro recebe o valor de retorno da função metodo_OF1_otimizado.
        Bloco_lista_erro_estimacao_parametro = metodo_OF1_otimizado(parametro, n_janelamento, bloco_treino_pulsos_sinais, bloco_teste_pulsos_sinais, bloco_teste_parametro_referencia, valor_minimo_amplitude_estimada_processo_fase, bloco_treino_amplitude_referencia)
    ...
    arquivo_saida_dados_estatisticos_k_fold_erro_OF1_otimizado(parametro, n_ocupacao, n_janelamento, media_media_blocos_erro_estimacao_parametro, var_media_blocos_erro_estimacao_parametro, DP_media_blocos_erro_estimacao_parametro, dado = "media")
    ...
    arquivo_saida_dados_estatisticos_k_fold_erro_OF1_otimizado(parametro, n_ocupacao, n_janelamento, media_var_blocos_erro_estimacao_parametro, var_var_blocos_erro_estimacao_parametro, DP_var_blocos_erro_estimacao_parametro, dado = "var")
    ...
    arquivo_saida_dados_estatisticos_k_fold_erro_OF1_otimizado(parametro, n_ocupacao, n_janelamento, media_DP_blocos_erro_estimacao_parametro, var_DP_blocos_erro_estimacao_parametro, DP_DP_blocos_erro_estimacao_parametro, dado = "DP")
...
def principal_K_fold_OF1_otimizado():
    ...
    for n_ocupacao in tqdm(range(ocupacao_inicial, ocupacao_final+1, incremento_ocupacao)):
        ...
        for n_janelamento in tqdm(range(n_janelamento_inicial, n_janelamento_final+1, incremento_janelamento)):
            ...
            K_fold_OF2_otimizado(parametro_amplitude, n_ocupacao, n_janelamento, Matriz_Pulsos_Sinais_Amplitude_Janelado, vetor_amplitude_referencia_janelado, valor_minimo_amplitude_estimada_processo_fase = None, vetor_amplitude_referencia_janelado = None)
            
            K_fold_OF2_otimizado(parametro_fase_amplitude_estimada, n_ocupacao, n_janelamento, Matriz_Pulsos_Sinais_Fase_Janelado, vetor_fase_referencia_janelado, valor_minimo_amplitude_estimada_processo_fase, vetor_amplitude_referencia_janelado = None)
            
            K_fold_OF2_otimizado(parametro_fase_amplitude_referencia, n_ocupacao, n_janelamento, Matriz_Pulsos_Sinais_Fase_Janelado, vetor_fase_referencia_janelado, valor_minimo_amplitude_estimada_processo_fase, vetor_amplitude_referencia_janelado)
...
principal_K_fold_OF1_otimizado() 
