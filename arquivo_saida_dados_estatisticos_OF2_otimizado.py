...
# Objetivo do código: geração de arquivos de saída baseados nos dados estatísticos dos histogramas do erro de estimação da amplitude, fase ou pedestal pelo método OF2 otimizado.
...
1) Função para o cálculo dos dados estatísticos do erro de estimação pelo método OF2 otimizado.
...
titulo_programa = colored("Geração de arquivos de saída baseados nos dados estatísticos dos histogramas do erro de estimação da amplitude, fase, ou pedestal pelo método Optimal Filtering (OF2 Otimizado):\n", attrs=["bold"])
...
### --- 2) INSTRUÇÃO PARA A IMPRESSÃO DOS DADOS ESTATÍSTICOS DO ERRO DE ESTIMAÇÃO DA AMPLITUDE, FASE OU PEDESTAL EM UM ARQUIVO DE SAÍDA PELO MÉTODO OF2 OTIMIZADO --- ###

# Definição da instrução para a impressão em um arquivo de saída, os dados estatísticos do erro de estimação do parâmetro pelo método OF2 otimizado.
def arquivo_saida_dados_estatisticos_erro_estimacao_parametro_OF2_otimizado(parametro, n_ocupacao, n_janelamento, media_erro_estimacao_parametro, var_erro_estimacao_parametro, desvio_padrao_erro_estimacao_parametro):
    ...
    arquivo_saida = f"dados_estatisticos_OF2_otimizado_{parametro}_janelamento_{n_janelamento}.txt"
...
def principal_arquivo_saida_dados_estatisticos_OF2_otimizado():
    ...
    for n_janelamento in tqdm(range(n_janelamento_inicial, n_janelamento_final+1, incremento_janelamento)):
        ...
        for n_ocupacao in tqdm(range(ocupacao_inicial, ocupacao_final+1, incremento_ocupacao)):
            ...
            lista_erro_estimacao_amplitude = metodo_OF2_otimizado(parametro_amplitude, n_janelamento, Matriz_Pulsos_Sinais_Treino_Janelado, Matriz_Pulsos_Sinais_Teste_Janelado, vetor_amplitude_referencia_teste_janelado, valor_minimo_amplitude_processo_fase = None, vetor_amplitude_referencia_treino_janelado = None)
            lista_erro_estimacao_fase_amplitude_estimada = metodo_OF2_otimizado(parametro_fase_amplitude_estimada, n_janelamento, Matriz_Pulsos_Sinais_Treino_Janelado, Matriz_Pulsos_Sinais_Teste_Janelado, vetor_fase_referencia_teste_janelado, valor_minimo_amplitude_processo_fase, vetor_amplitude_referencia_treino_janelado = None)
            lista_erro_estimacao_fase_amplitude_referencia = metodo_OF2_otimizado(parametro_fase_amplitude_referencia, n_janelamento, Matriz_Pulsos_Sinais_Treino_Janelado, Matriz_Pulsos_Sinais_Teste_Janelado, vetor_fase_referencia_teste_janelado, valor_minimo_amplitude_processo_fase, vetor_amplitude_referencia_treino_janelado)
            lista_erro_estimacao_pedestal = metodo_OF2_otimizado(parametro_pedestal, n_janelamento, Matriz_Pulsos_Sinais_Treino_Janelado, Matriz_Pulsos_Sinais_Teste_Janelado, vetor_pedestal_referencia_teste_janelado, valor_minimo_amplitude_processo_fase = None, vetor_amplitude_referencia_treino_janelado = None)
            ...
            media_erro_estimacao_fase_amplitude_estimada, var_erro_estimacao_fase_amplitude_estimada, desvio_padrao_erro_estimacao_fase_amplitude_estimada = dados_estatisticos_erro_estimacao_parametro_OF2_otimizado(lista_erro_estimacao_fase_amplitude_estimada)
            media_erro_estimacao_fase_amplitude_referencia, var_erro_estimacao_fase_amplitude_referencia, desvio_padrao_erro_estimacao_fase_amplitude_referencia = dados_estatisticos_erro_estimacao_parametro_OF2_otimizado(lista_erro_estimacao_fase_amplitude_referencia)
            ...
            arquivo_saida_dados_estatisticos_erro_estimacao_parametro_OF2_otimizado(parametro_fase_amplitude_estimada, n_ocupacao, n_janelamento, media_erro_estimacao_fase_amplitude_estimada, var_erro_estimacao_fase_amplitude_estimada, desvio_padrao_erro_estimacao_fase_amplitude_estimada)
            arquivo_saida_dados_estatisticos_erro_estimacao_parametro_OF2_otimizado(parametro_fase_amplitude_referencia, n_ocupacao, n_janelamento, media_erro_estimacao_fase_amplitude_referencia, var_erro_estimacao_fase_amplitude_referencia, desvio_padrao_erro_estimacao_fase_amplitude_referencia)
